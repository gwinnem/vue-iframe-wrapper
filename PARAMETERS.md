# IframeWrapper — Complete Parameter Reference

This document is the exhaustive reference for every prop, event, exposed method, and
supporting type on `<IframeWrapper />`. For a narrative introduction see
[Getting Started](./docs/guide/getting-started.md); for runnable snippets see
[Examples](./docs/guide/examples.md); for the underlying native `<iframe>` element these
props wrap (every native attribute, sandbox token, and permission directive), see
[`IFRAME_HTML_REFERENCE.md`](./IFRAME_HTML_REFERENCE.md). This same content is published
as a page in the VitePress site at **API → Parameters**.
...
- [Props](#props)
  - [Content props](#content-props) — `src`, `srcdoc`
  - [Appearance props](#appearance-props) — `iframeClass`, `iframeStyle`, `sandbox`, `title`
  - [Asset props](#asset-props) — `css`, `cssUrls`, `js`, `jsUrls`
  - [Behaviour props](#behaviour-props) — `autoInject`, `injectOnLoad`, `reloadKey`
- [Events](#events)
- [Exposed instance methods](#exposed-instance-methods)
- [`InjectionOptions`](#injectionoptions)
- [`IframeInjectionResult`](#iframeinjectionresult)
- [`IframeError`](#iframeerror)
- [Prop interactions & precedence rules](#prop-interactions--precedence-rules)
...
scripts to execute. See [Security](./docs/security.md) for the full threat model before
...
like any other code you ship to users — see [Security](./docs/security.md).
...
- **CSS defaults to `head`, JS defaults to `body`** when no `placement` is given via
  ...
  imperative API (see [Feature Recommendations](./FEATURE_RECOMMENDATIONS.md) for a
