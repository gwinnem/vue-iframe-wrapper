...
`InjectionOptions` API (see [Parameters → Prop interactions](./PARAMETERS.md#prop-interactions--precedence-rules)).
...
[Examples → Manual API](./docs/guide/examples.md#manual-api)), which works but requires
...
- A sandboxed execution environment for genuinely untrusted third-party HTML (this
  library previews content _you_ control — see [Security](./docs/security.md)).
